...
from shopify_api import make_shopify_request, fetch_products, update_image_alt_text, fetch_selected_products
...
# Help tab
    ...
    st.write("Welcome to the Shopify Alt Text Manager help section.")
    ...
    selected_topic = st.selectbox("Select a topic", options=list(help_topics.keys()), format_func=lambda x: help_topics[x])
    
    st.subheader(help_topics[selected_topic])
    
    if selected_topic == "getting_started":
        st.markdown(guides["getting_started"])
    elif selected_topic == "connecting":
        st.markdown(guides["connection_guide"])
    elif selected_topic == "templates":
        st.markdown(guides["template_guide"])
        ...
        ```
        {title} by {vendor} - {type} from {store}
        ```
        
        ```
        {brand} {color} {category} - {sku}
        ```
        """)
