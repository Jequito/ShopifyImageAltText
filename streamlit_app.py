...
# Templates tab
    ...
    # Products guide
    with st.expander("📋 Product Management Guide", expanded=len(st.session_state.products) == 0):
    ...
    if st.session_state.products:
        ...
        if product_data:
            ...
            # Show product table with a view details button
    ...
    if st.session_state.current_product:
        ...
        # Template selector for bulk apply
        st.subheader("Bulk Apply Template")
        ...
        with col1:
            if st.session_state.templates:
                ...
                st.info("No templates available. Create templates in the Templates tab.")
        ...
        # Display images with alt text editing
                    ...
                    if st.session_state.templates:
                        ...
                        if selected:
                            if st.button("Apply", key=f"apply_{image['id']}"):
                                ...
                                st.success(f"Template applied: {new_alt_text}")
                                st.rerun()
                    
                    # Button to clear alt text
    ...
    if not st.session_state.shopify_connected:
        ...
        with st.expander("Run Comprehensive Diagnostics", expanded=True):
